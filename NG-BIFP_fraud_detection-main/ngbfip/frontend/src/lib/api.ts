...
export const transactionAPI = {
  list: (page = 1, pageSize = 20, filters?: any) =>
    api.get<TransactionListResponse>('/transactions/', {
      params: { skip: (page - 1) * pageSize, limit: pageSize, ...filters },
    }),
  ...
  search: (query: string) => api.get<Transaction[]>('/transactions/search', { params: { q: query } }),
  getHistory: (userId: number) => api.get<Transaction[]>(`/transactions/user/${userId}`),
  getStats: () => api.get<TransactionStats>('/dashboard/overview'),
  importCSV: (file: File) => {
...
export const dashboardAPI = {
  ...
  getRecentTransactions: (limit = 10) => api.get<any>(`/dashboard/recent-transactions?limit=${limit}`),
  getDailySummary: (days = 7) => api.get<any>(`/dashboard/daily-summary?days=${days}`),
  getFraudTrends: (days = 30) => api.get<any>(`/dashboard/fraud-trends?days=${days}`),
  getRiskDistribution: () => api.get<any>('/dashboard/risk-distribution'),
}
