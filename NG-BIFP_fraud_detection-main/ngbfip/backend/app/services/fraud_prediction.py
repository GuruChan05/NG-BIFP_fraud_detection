...
class FraudPredictionService:
    ...
    def predict_fraud(transaction: Transaction) -> dict:
        """Simple rule-based fraud prediction (placeholder for real ML model)."""
        # Returns risk_score in 0.0–1.0 scale
        raw_score = FraudPredictionService._calculate_risk_score(transaction)
        risk_score = round(raw_score / 100.0, 4)  # Normalize to 0.0-1.0
        
        # Determine prediction based on risk score
        if risk_score >= 0.70:
            prediction = 'fraudulent'
            confidence = min(0.95, 0.5 + (risk_score / 2))
        else:
            prediction = 'legitimate'
            confidence = min(0.95, 0.5 + ((1.0 - risk_score) / 2))
        
        explanation = FraudPredictionService._generate_explanation(
            transaction, raw_score
        )
