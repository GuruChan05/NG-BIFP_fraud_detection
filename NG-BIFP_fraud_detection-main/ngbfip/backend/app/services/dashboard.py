...
class DashboardService:
    ...
    def get_overview(db: Session) -> dict:
        ...
        # High risk transactions (risk_score is 0.0-1.0 scale)
    ...
    def get_summary_stats(db: Session) -> dict:
        ...
        # Risk distribution (risk_score is 0.0-1.0 scale)
        ...
        medium_risk = db.query(func.count(Transaction.id)).filter(
            and_(Transaction.risk_score >= 0.30, Transaction.risk_score < 0.70)
        ).scalar() or 0
        high_risk = db.query(func.count(Transaction.id)).filter(
            Transaction.risk_score >= 0.70
        ).scalar() or 0
    ...
    def get_risk_distribution(db: Session) -> dict:
        ...
        return {
            'very_low': db.query(func.count(Transaction.id)).filter(
                Transaction.risk_score < 0.20
            ).scalar() or 0,
            'low': db.query(func.count(Transaction.id)).filter(
                and_(Transaction.risk_score >= 0.20, Transaction.risk_score < 0.40)
            ).scalar() or 0,
            'medium': db.query(func.count(Transaction.id)).filter(
                and_(Transaction.risk_score >= 0.40, Transaction.risk_score < 0.60)
            ).scalar() or 0,
            'high': db.query(func.count(Transaction.id)).filter(
                and_(Transaction.risk_score >= 0.60, Transaction.risk_score < 0.80)
            ).scalar() or 0,
            'very_high': db.query(func.count(Transaction.id)).filter(
                Transaction.risk_score >= 0.80
            ).scalar() or 0,
            'total': total
        }
